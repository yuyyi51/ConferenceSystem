...
module.exports = class Mongo {
  ...
  static register(username, password, institution, fn) {
    this.findUser(username, (res) => {
      if (res) {
        fn(false);
        return;
      }
      db.user.insert({username: username, password: password, institution:institution}, (err, res) => {
        if (!res) {
          fn(false);
          return;
        }
